...
class TestGetUptimeReport:
    ...
    def test_formatting(
        self, mocker, mock_message_bus, mock_error_parser, startup_time, now, expected
    ):
    ...
    def test_startup_in_future(self, mocker, monitor):
        mock_dt = mocker.patch("src.monitoring.monitor.datetime", wraps=datetime)
    ...
    def test_datetime_now_raises(self, mocker, monitor):
        mock_dt = mocker.patch("src.monitoring.monitor.datetime", wraps=datetime)
...
class TestGetRecentItemsReport:
    async def test_uses_configured_hours(self, monitor, mock_message_bus):
    ...
    async def test_different_timeframe(
        self,
        mock_message_bus,
        mock_error_parser,
    ):
    ...
    async def test_with_results(self, monitor, mock_message_bus):
        mock_message_bus.query.return_value = [
    ...
    async def test_empty(self, monitor, mock_message_bus):
        mock_message_bus.query.return_value = []

        result = await monitor._get_recent_items_report()
        assert "(last 1h): 0 items" in result

    async def test_none(self, monitor, mock_message_bus):
        mock_message_bus.query.return_value = None

        result = await monitor._get_recent_items_report()
        assert "(last 1h): 0 items" in result

    async def test_single_result(self, monitor, mock_message_bus):
        mock_message_bus.query.return_value = [
            {"search_id": 1, "query": "nike shoes", "item_count": 5},
        ]
    ...
    async def test_zero_count(self, monitor, mock_message_bus):
        mock_message_bus.query.return_value = [
    ...
    async def test_query_with_html_characters_is_escaped(
        self, monitor, mock_message_bus
    ):
        mock_message_bus.query.return_value = [
    ...
    async def test_query_exception(self, monitor, mock_message_bus):
...
class TestGetProxyReport:
    def test_not_configured(self, monitor):
        result = monitor._get_proxy_report()
        assert result == "🔒 Proxies: Not configured"

    def test_all_healthy(self, monitor_with_proxy, mock_proxy_manager):
    ...
    def test_some_banned(self, monitor_with_proxy, mock_proxy_manager):
        mock_proxy_manager.proxies = [MagicMock(), MagicMock(), MagicMock()]
    ...
    def test_all_banned(self, monitor_with_proxy, mock_proxy_manager):
...
class TestGetRecentErrorsReport:
    async def test_with_results(self, monitor, mock_error_parser):
    ...
    async def test_empty(self, monitor, mock_error_parser):
        mock_error_parser.get_recent_errors.return_value = []

        result = await monitor._get_recent_errors_report()
        assert result == "⚠️ Recent Errors: None"

    async def test_exception(self, monitor, mock_error_parser):
...
class TestGenerateStatusReport:
    async def test_full_report_structure(
        self, mocker, monitor, mock_message_bus, mock_error_parser
    ):
        mock_dt = mocker.patch("src.monitoring.monitor.datetime", wraps=datetime)
    ...
    async def test_with_all_errors(
        self, mocker, monitor, mock_message_bus, mock_error_parser
    ):
        mock_dt = mocker.patch("src.monitoring.monitor.datetime", wraps=datetime)
