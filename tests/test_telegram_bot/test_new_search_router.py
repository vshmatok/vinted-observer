...
async def test_cmd_new_search_clears_state(mock_message, mock_fsm_context):
    await cmd_new_search(mock_message, mock_fsm_context)

    mock_fsm_context.clear.assert_called_once()


async def test_cmd_new_search_sets_waiting_for_search_term(
...
async def test_cmd_new_search_sends_prompt_with_cancel_keyboard(
    mock_message, mock_fsm_context
):
    await cmd_new_search(mock_message, mock_fsm_context)
...
async def test_cancel_create_clears_state(mock_message, mock_fsm_context):
    await cancel_create(mock_message, mock_fsm_context)

    mock_fsm_context.clear.assert_called_once()


async def test_cancel_create_sends_cancellation_and_removes_keyboard(
    mock_message, mock_fsm_context
):
    await cancel_create(mock_message, mock_fsm_context)
...
async def test_add_query_no_text_sends_prompt(mock_message, mock_fsm_context):
...
async def test_add_query_invalid_query_sends_error(mock_message, mock_fsm_context):
...
async def test_add_query_min_length_boundary(mock_message, mock_fsm_context):
...
async def test_add_query_exceeds_max_length(mock_message, mock_fsm_context):
...
async def test_add_query_whitespace_only_sends_error(mock_message, mock_fsm_context):
    mock_message.text = "   "
...
async def test_add_query_valid_query(mock_message, mock_fsm_context):
...
async def test_add_query_strips_whitespace(mock_message, mock_fsm_context):
    mock_message.text = "  nike shoes  "
...
async def test_add_query_error_clears_state(mock_message, mock_fsm_context):
    mock_message.text = "nike shoes"
...
async def test_add_price_min_no_text_sends_prompt(mock_message, mock_fsm_context):
    mock_message.text = None
...
async def test_add_price_min_non_numeric_sends_error(mock_message, mock_fsm_context):
...
async def test_add_price_min_negative_sends_error(mock_message, mock_fsm_context):
    mock_message.text = "-5"
...
async def test_add_price_min_zero_valid(mock_message, mock_fsm_context):
    mock_message.text = "0"
...
async def test_add_price_min_decimal_valid(mock_message, mock_fsm_context):
...
async def test_add_price_min_valid(mock_message, mock_fsm_context):
...
async def test_add_price_min_error_clears_state(mock_message, mock_fsm_context):
    mock_message.text = "50"
    mock_message.answer.side_effect = TelegramAPIError(
        method=MagicMock(), message="error"
    )
...
async def test_add_price_max_no_text_sends_prompt(
    mock_message, fsm_with_data, mock_message_bus
):
    mock_message.text = None
...
async def test_add_price_max_non_numeric_sends_error(
    mock_message, fsm_with_data, mock_message_bus
):
    mock_message.text = "abc"
...
async def test_add_price_max_negative_sends_error(
    mock_message, fsm_with_data, mock_message_bus
):
...
async def test_add_price_max_invalid_range(
    mock_message, fsm_with_data, mock_message_bus
):
...
async def test_add_price_max_equal_to_min_rejected(
    mock_message, fsm_with_data, mock_message_bus
):
...
async def test_add_price_max_missing_query_in_state(
...
async def test_add_price_max_missing_price_min_in_state(
    mock_message, mock_fsm_context, mock_message_bus
):
    mock_message.text = "100"
...
async def test_add_price_max_empty_state_data(
    mock_message, mock_fsm_context, mock_message_bus
):
    mock_message.text = "100"
    mock_fsm_context.get_data.return_value = {}
...
async def test_add_price_max_executes_command(
    mock_message, fsm_with_data, mock_message_bus, successful_execute
):
    mock_message.text = "100"
...
async def test_add_price_max_sends_success_message(
    mock_message, fsm_with_data, mock_message_bus, successful_execute
):
    mock_message.text = "100"
...
async def test_add_price_max_publishes_event(
    mock_message, fsm_with_data, mock_message_bus, successful_execute
):
    mock_message.text = "100"
...
async def test_add_price_max_clears_state_on_success(
    mock_message, fsm_with_data, mock_message_bus, successful_execute
):
    mock_message.text = "100"
...
async def test_add_price_max_command_error_returns_early(
    mock_message, fsm_with_data, mock_message_bus
):
    mock_message.text = "100"
...
async def test_add_price_max_build_message_fallback(
    mock_message, fsm_with_data, mock_message_bus, successful_execute, mocker
):
    mock_message.text = "100"
    mocker.patch(
...
async def test_add_price_max_outer_exception_clears_state(
    mock_message, mock_fsm_context, mock_message_bus
):
    mock_message.text = "100"
