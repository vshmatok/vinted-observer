...
from tests.test_vinted_network_client.conftest import make_item_json
...
def test_full_json_parses_all_fields():
...
def test_user_parsed_as_vinted_user():
    item = VintedItem(make_item_json())
    assert isinstance(item.user, VintedUser)
    assert item.user.login == "seller123"


def test_invalid_user_sets_none():
    item = VintedItem(make_item_json(user=123))
    assert isinstance(item.user, VintedUser)
    assert item.user.id is None


def test_photo_parsed_as_vinted_image():
    item = VintedItem(make_item_json())
    assert isinstance(item.photo, VintedImage)


def test_invalid_photo_sets_none():
    item = VintedItem(make_item_json(photo=123))
    assert isinstance(item.photo, VintedImage)
    assert item.photo.id is None


def test_price_from_dict_format():
    item = VintedItem(
        make_item_json(
...
def test_non_vinted_item_returns_false():
    a = VintedItem(make_item_json(id=1))
...
def test_different_id_different_hash():
    a = VintedItem(make_item_json(id=1))
    b = VintedItem(make_item_json(id=2))
    assert hash(a) != hash(b)
...
def test_str_minimal_item_with_id():
    item = VintedItem({"id": 42})
    assert "42" in str(item)


def test_str_empty_item():
    item = VintedItem()
...
def test_repr_has_balanced_parentheses():
    item = VintedItem(make_item_json())
    result = repr(item)
...
# ============================================================================
# __eq__ / __hash__ edge cases
# ============================================================================


def test_both_id_none_are_equal():
    a = VintedItem()
    b = VintedItem()
    assert a == b


def test_both_id_none_same_hash():
    a = VintedItem()
    b = VintedItem()
    assert hash(a) == hash(b)


def test_id_none_deduplication_in_set():
    a = VintedItem()
    b = VintedItem()
...
# ============================================================================
# created_at_ts edge cases
# ============================================================================


def test_created_at_ts_extreme_negative_timestamp():
...
def test_total_item_price_without_price_key():
    json_data = make_item_json(
...
def test_buy_url_splits_at_first_items_occurrence():
