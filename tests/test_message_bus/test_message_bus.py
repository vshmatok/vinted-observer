...
class TestRegisterEvent:
    async def test_register_and_publish(self, message_bus: MessageBus):
    ...
    async def test_already_registered_raises(self, message_bus: MessageBus):
        ...
        with pytest.raises(
            ValueError,
            match=f"Event handler already registered for {MockEvent.__name__}",
        ):
            message_bus.register_event(MockEvent, event_handler)


class TestPublish:
    async def test_unregistered_event_raises(self, message_bus: MessageBus):
    ...
    async def test_exception_does_not_propagate(self, message_bus: MessageBus):
    ...
    async def test_handler_receives_correct_type(self, message_bus: MessageBus):
...
class TestRegisterCommand:
    async def test_register_and_execute(self, message_bus: MessageBus):
    ...
    async def test_with_result(self, message_bus: MessageBus):
    ...
    async def test_with_none_result(self, message_bus: MessageBus):
...
class TestExecute:
    async def test_unregistered_command_raises(self, message_bus: MessageBus):
        """Test executing an unregistered command raises error."""

        test_command = MockCommand(value=1)
        with pytest.raises(
            ValueError,
            match=f"No command handler registered for {MockCommand.__name__}",
        ):
            await message_bus.execute(test_command)

    async def test_exception_propagates(self, message_bus: MessageBus):
...
class TestRegisterQuery:
    async def test_register_and_query(self, message_bus: MessageBus):
...
class TestQuery:
    async def test_unregistered_query_raises(self, message_bus: MessageBus):
